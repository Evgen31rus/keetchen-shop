...
export default function Modal({ props, propsMobile }: propsTypes) {
  ...
  return (
    ...
    ${
      state.isOpenCostСalculation ||
      state.isOpenConsultation ||
      state.isOpenMobileMenu
        ? " translate-x-[-50%]  translate-y-[-50%]"
        : "translate-x-[-180%]  translate-y-[200vh]"
    }
    ${
      state.isOpenCostСalculation || state.isOpenMobileMenu
        ? "w-[70%] max-w-[800px] h-[95vh] sm:w-[90%]"
        : "w-[30%] h-[30vh] md:w-[90%] sm:w-[90%] sm:h-[35vh]"
    }
    ...
    >
      {!state.isOpenMobileMenu ? (
        state.isOpenCostСalculation && !state.isOpenConsultation ? (
          <ModalCostСalculation
            props={props}
            propsSVG={propsMobile.svg}
            propsRoomArea={propsMobile.roomArea}
          />
        ) : (
          <ModalConsultation />
        )
