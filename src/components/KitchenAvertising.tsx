...
export default function KitchenAvertising() {
  return (
    <div className={`flex flex-col w-[80%] items-start m-auto mt-`}>
        ...
        <h1 className={`  w-[80%]  text-[2.5rem] font-extrabold`}>Почему выбирают <span className={`text-[#E3010F] bold`}> наши кухни?</span> </h1>
...
sm:flex-col
`}>
<MainButton textNotActive={`Рассчитать стоимость`} width={30} widthMobile={80} heigt={40} heightMobile={50} bgColor={`#E3010F`} colorText={`white`} margin={5} />
<MainButton textNotActive={`Получить весь каталог`}  width={30} widthMobile={80} heigt={40} heightMobile={50} isBorder={true} margin={5}/>
</div>
</div>
    </div>
  );
