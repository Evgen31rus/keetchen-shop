...
export default function Form({
  ...
  return (
    <div
      className={` w-[100%]  h-[100%] justify-center rounded-2xl  
    ...
    >
      {state.validFormSlice.isValidForm ? (
        ...
        >
          <div ref={modalRef}>
            <div
              className={`flex  w-[100%]  h-[100%] rounded relative flex flex-col items-center p-5 mt-5 
sm:flex sm:flex-col
${bannerZone ? "items-center" : ""}
`}
            >
              <p
                className={` flex text-[1.2rem] w-[90%] sm:flex  sm:w-[80%] text-center

`}
              >
                Заявка оформлена, скоро с вами свяжется наш специалист.
              </p>

              <div
                className={`absolute flex justify-center items-center rounded-full w-[60px] h-[60px]  top-menu-shadow top-[30%] sm:top-[55%]
...
`}
        ...
        >
          <div
            className={`  justify-around  h-[100%] rounded-2xl items-center  ${
              !MobileVisible ? "flex flex-col" : "hidden"
            }
...
`}
      ...
      `}
            ...
            <p
              className={`
    ${!validForm.numberLength ? "h-[10px] opacity-1" : "h-[0px] opacity-0"}
    text-[#E3010F] mb-2 transition-all`}
            >
              Номер менее 11 цифр
            </p>
            <div className={`flex w-[80%] `}>
              <div
                className={`  border-[#E3010F] border-2   flex items-center justify-around rounded
    ${
      bannerZone
        ? "w-[60px] h-[25] text-[1.2rem]"
        : "w-[90px] h-[40px] text-[2rem]"
    }
    `}
              >
                <svg
    ...
    `}
              ...
              <input
                type="text"
                className={`w-[80%]  border-[2px] border-l-0 border-[#E3010F]  rounded outline-0 pl-5 pr-5 text-center 
